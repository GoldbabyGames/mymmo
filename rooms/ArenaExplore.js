...
class ArenaExplore {
    ...
    checkForCombat(sessionId, position) {
		const adjacentPlayers = Array.from(this.room.state.players.values())
			.filter(p => {
				if (p.id === sessionId) return false;
				
				// Check if players are on adjacent tiles
				const rowDiff = Math.abs(p.row - position.row);
				const colDiff = Math.abs(p.col - position.col);
				
				// Players are adjacent if they differ by 1 in exactly one dimension
		...
		if (adjacentPlayers.length > 0) {
			...
			// Log combat trigger for debugging
			console.log("Combat triggered between players:", {
				player1: sessionId,
				player2: adjacentPlayers[0].id,
